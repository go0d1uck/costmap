...
bool Costmap::TestMap(const std::vector<std::vector<bool>>& m) {
  ...
  while (!q.empty()) {
    ...
    if (m[t.first][t.second] == 1) {
      LOG(INFO) << "collision position" << t.first << "," << t.second
                << "--dis:"
                << std::hypot(center_i - t.first, center_j - t.second);
      return false;
    } else {
...
Costmap::Costmap(double robot_x, double robot_y, double robot_yaw,
  ...
  try {
    std::vector<std::vector<bool>> local_map(
        map_size_ / RESOLUTION,
        std::vector<bool>(map_size_ / RESOLUTION, false));
