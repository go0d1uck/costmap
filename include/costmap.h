 ...
 private:
  ...
  struct Cell {
    ...
    Cell(int x, int y, int tx, int ty, int d)
        : mx(x), my(y), src_x(tx), src_y(ty), dis(d) {}
    bool operator<(const Cell a) const { return dis > a.dis; }
  };
  void TailMapAInMapB(const std::vector<std::vector<bool>>& src_map,
                      std::vector<std::vector<bool>>& des_map);
  void EnQueue(int x, int y, int src_x, int src_y,
               std::vector<std::vector<bool>>& seen, std::priority_queue<Cell>&,
               std::vector<std::vector<unsigned char>>& cost_map);
  Costmap(double robot_x, double robot_y, double robot_yaw,
...
#endif
 public:
  void getGloblPos(const int& x, const int& y, double& globl_x,
                   double& glob_y) {
    std::pair<double, double> origin;
    origin.first = getOriginX(), origin.second = getOriginY();
    globl_x = origin.first - x * getResolution(),
    glob_y = origin.second - y * getResolution();
  }
  std::vector<std::vector<bool>> GetLayeredMap(double new_robot_x,
                                               double new_robot_y,
                                               double new_robot_yaw);
  static Costmap& getInstance(std::string file_name) {
    static Costmap instance_ = Costmap(0.0, 0.0, 0.0, file_name);
    return instance_;
  }
  static Costmap& getInstance() { return getInstance(""); }
  std::vector<std::vector<unsigned char>> UpdateCostMap(double robot_x,
                                                        double robot_y,
                                                        double robot_yaw);
  bool AddPlug(std::vector<std::vector<bool>>& gridMap, std::string name,
 ...
 protected:
  inline unsigned char ComputeCost(double distance) const {
    ...
    if (distance == 0) {
      ...
      double factor =
          std::exp(-1.0 * inflation_weight_ * (distance - inscribed_radius_));
  ...
  inline unsigned char GetCost(Cell t) {
    ...
    double result =
        fmod(robot_yaw_ - obstacle_angle_in_robot + M_PI, 2.0 * M_PI);
    result = (result <= 0) ? (result + M_PI) : (result - M_PI);
    bool right_toward_in_robot = result > 0;
    right_toward_in_robot = false;
    if (dis_mile > inflation_radius_)
      return 0;
    else {
      unsigned char t =
          ComputeCost(right_toward_in_robot ? (dis_mile * 1.3) : dis_mile);
      if (t < 10)
        return 0;
      else
        return t;
